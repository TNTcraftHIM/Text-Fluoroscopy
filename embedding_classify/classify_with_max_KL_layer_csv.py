...
test_embeddings            = torch.load(f'save/{which_embedding}_embedding/save_embedding/test_data.pt')
...
test_labels                = torch.load('dataset/labels/test_data.pt').to(device)
...
def test(model,test_set,test_label,test_acc,testset_name):
    with torch.no_grad():
        ...
        fpr, tpr, thresholds = roc_curve(test_label.cpu().numpy(), probabilities.cpu().numpy())
        ...
        "ALL_AUROC: {:.4f}; ALL_AVG_FPR: {:.4f}; ALL_STD_FPR: {:.4f}; ALL_AVG_TPR: {:.4f}; ALL_STD_TPR: {:.4f}".format(
            auroc, all_avg_fpr, all_std_fpr, all_avg_tpr, all_std_tpr))
    return auroc
...
def train(hidden_sizes,droprate):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, Validation Accuracy: {accuracy:.4f}, test_data Test auroc: {','.join([str(round(i,4)) for i in testset_acc])}")
